...
# mejorado: ahora los elementos se sortean primero y se agregan los de menor duración al inicio
def fsolve(n, m, L, p, tries = 100, consider = 2):
    ...
    for _ in range(tries):
        ...
        while len(rcl) > 0:
            def comp(job_machine):
                j, k = job_machine
                return p[j][k]

            rcl.sort(key=comp)
            job, machine = rcl.pop(random.randint(0, min(consider, len(rcl) - 1)))
